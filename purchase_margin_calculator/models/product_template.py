...
class ProductTemplate(models.Model):
    ...
    last_purchase_uom_id = fields.Many2one(
        'uom.uom',
        string='Purchase UoM',
        readonly=True,
    )
    ...
    # --- Converted Cost & Margin ---
    last_cost_converted = fields.Float(
        string='Last Cost (Sale UoM)',
        digits='Product Price',
        readonly=True,
    )
    effective_sale_price = fields.Float(
        string='Effective Sale Price',
        compute='_compute_effective_sale_price',
        digits='Product Price',
    )
    ...
    price_alert = fields.Selection(
        [('ok', 'OK'), ('warning', 'Warning'), ('critical', 'Critical'), ('no_data', 'No Data')],
        string='Price Alert',
        readonly=True,
    )
    margin_vs_list = fields.Float(
        string='Margin vs List',
        digits=(5, 4),
        readonly=True,
    )
    ...
    purchase_price_history_ids = fields.One2many(
        'purchase.price.history', 'product_tmpl_id',
    ...
    def _compute_effective_sale_price(self):
        """Get sale price: POS pricelist if available, else list_price."""
        pricelist = None
    ...
    def _get_margin_factor(self):
        param = self.env['ir.config_parameter'].sudo().get_param(
            'purchase_margin_calculator.margin_factor', '1.30',
        )
        try:
            return float(param)
    ...
    def _convert_uom_price(purchase_uom, product_uom, price_unit):
        """Convert price from purchase UoM to product UoM."""
    ...
    def _update_last_purchase_cost(self):
        """Recalculate last purchase cost fields from confirmed purchase orders."""
        ...
        for product in self:
            ...
            if not variant_ids:
                product.write({
                    ...
                    'last_purchase_uom_id': False,
                    'last_purchase_order_id': False,
                    'last_cost_converted': 0,
                    'suggested_price': 0,
                    'price_alert': 'no_data',
                    'margin_vs_list': 0,
                })
            ...
            converted = self._convert_uom_price(
                line.product_uom_id, product.uom_id, price_unit,
            )
            ...
            product.write({
                ...
                'last_purchase_uom_id': line.product_uom_id.id if line.product_uom_id else False,
    ...
    # -------------------------------------------------------------------------
    # Populate history from existing POs (runs on install + manual button)
    # -------------------------------------------------------------------------
    @api.model
    def _populate_history_from_existing(self):
        """Scan confirmed POs and create history entries for all products."""
        ...
        for tmpl in templates:
            ...
            # Skip if history already exists for this product
            ...
            for line in lines:
                converted = self._convert_uom_price(
                    line.product_uom_id, tmpl.uom_id, line.price_unit,
                )
                History.create({
        ...
        # Also recalculate last cost for all
        batch_size = 100
        ...
        _logger.info(
            'purchase_margin_calculator: Created %d history entries for %d products',
            count, len(templates),
        )
